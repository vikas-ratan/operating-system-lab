...
class less_than_srtf
{
    public:
    inline bool operator() (const PCB& p1, const PCB& p2)
    {
        if((p1.bt-p1.tempbt)==(p2.bt-p2.tempbt))
            return p1.at<p2.at;
        return (p1.bt-p1.tempbt)<(p2.bt-p2.tempbt);
    }
};
void SJFPreemp(deque<PCB> &P,deque<pair<int ,PCB > > &ganttChart,int n,int tq)
{
    deque<PCB> readyQ;
    ...
    while(!readyQ.empty())
    {
        ...
        sort(readyQ.begin(),readyQ.end(),less_than_srtf());
...
int main()
{
    ...
    SJFPreemp(P,ganttChart,n,PCB::tq);
    for(int i=0;i<P.size();i++)
    {
        PCB::avgWt +=P[i].wt;
        PCB::avgTat+=P[i].tat;
    ...
    cout<<"\tSJF(Preemptive) CPU SCHEDULING\n";
